...
function homeTeamLogo(team) {
  return team.slug ? `/assets/images/teams/home/${team.slug}.webp` : team.logo || "";
}

function teamLogo(teams, code, alt = "") {
  const team = teamByCode(teams, code);
  return `<img src="${escapeHtml(homeTeamLogo(team))}" alt="${escapeHtml(alt || `${team.name || code} logo`)}" width="76" height="76" loading="lazy" decoding="async" />`;
}
...
function renderNextMatch(site, teams, fixtures) {
  ...
  return `<section class="rh-next-match" aria-labelledby="rh-next-match-title">
    <div class="rh-next-match-main">
      <div class="rh-match-intro"><div><p class="rh-match-kicker"><i data-lucide="circle-arrow-right" aria-hidden="true"></i> Opening fixture</p><h3 id="rh-next-match-title">Next Tournament Match</h3></div><p><i data-lucide="clock-3" aria-hidden="true"></i> Venue local time</p></div>
...
function renderLiveScore(site, teams, fixtures) {
  ...
  return `<section class="rh-section rh-live-score" aria-labelledby="rh-live-score-title">
    <div class="rh-section-title"><div><p class="rh-eyebrow">Match status</p><h3 id="rh-live-score-title">Live Match Scores</h3></div><a href="/live-score/">Open live score <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
...
function renderUpcoming(fixtures, teams) {
  return `<section class="rh-section rh-upcoming" aria-labelledby="rh-upcoming-title">
    <div class="rh-section-title"><div><p class="rh-eyebrow">Next fixtures</p><h3 id="rh-upcoming-title">Upcoming Matches</h3><p class="rh-section-copy">Confirmed match dates, venues, and local start times.</p></div><a href="/fixtures/">View full schedule <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
...
function renderMatchCentre(site, teams, fixtures) {
  return `<section class="rh-match-centre" aria-labelledby="rh-match-centre-title">
    <div class="rh-match-centre-heading"><div><p class="rh-eyebrow">Fixtures and scores</p><h2 id="rh-match-centre-title">Tournament Match Centre</h2></div><a href="/fixtures/">Complete schedule <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
...
function renderTeams(teams, squadByTeam) {
  return `<section class="rh-section rh-teams" aria-labelledby="rh-teams-title">
    ...
    <div class="rh-team-grid">${teams.map((team) => {
      ...
      <img src="${escapeHtml(homeTeamLogo(team))}" alt="${escapeHtml(team.name)} team logo" width="112" height="112" loading="lazy" decoding="async" />
...
function renderStandings(teams) {
  return `<article class="rh-panel rh-standings" aria-labelledby="rh-standings-title">
    <div class="rh-panel-heading"><div><h2 id="rh-standings-title">Tournament Points Table</h2><p class="rh-section-copy">Tournament standings track wins, losses, points, and net run rate for playoff qualification.</p></div><a href="/points-table/">View full table <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
    ...
    <div class="rh-table-wrap"><table><thead><tr><th>Pos</th><th>Team</th><th>P</th><th>W</th><th>L</th><th>NRR</th><th>Pts</th></tr></thead><tbody>${teams.map((team, index) => `<tr><td>${index + 1}</td><th><img src="${escapeHtml(homeTeamLogo(team))}" alt="" width="28" height="28" loading="lazy" decoding="async" />${escapeHtml(team.name)}</th><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>`).join("")}</tbody></table></div>
...
function renderNews(news) {
  return `<article class="rh-panel rh-news" aria-labelledby="rh-news-title">
    <div class="rh-panel-heading"><div><h2 id="rh-news-title">Latest League News</h2><p class="rh-section-copy">Verified fixture changes, squad news, player profiles, and broadcast market updates.</p></div><a href="/news/">View all updates <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
    <div class="rh-news-list">${news.slice(0, 5).map((item) => `<a href="/news/${escapeHtml(item.slug)}/"><img src="/assets/images/news/thumbs/${escapeHtml(item.image.split("/").pop())}" alt="${escapeHtml(item.title)}" width="120" height="76" loading="lazy" decoding="async" /><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.date)}</small></span></a>`).join("")}</div>
  </article>`;
}
...
function renderFeatureTiles() {
  const tiles = [
    ...
    ["trophy", "Points Table", "Team standings, net run rate, and playoff races.", "/points-table/"],
    ["users", "Teams", "Franchise squads, captains, coaches, and home grounds.", "/teams/"],
    ["person-standing", "Players", "T20 player profiles, roles, and career stats.", "/players/"],
    ["landmark", "Venues", "Stadium guides, seating, locations, and match lists.", "/venues/"],
    ["tv", "How to Watch", "Confirmed TV channels and regional streaming links.", "/how-to-watch/"]
  ];
  return `<section class="rh-section rh-feature-tiles" aria-labelledby="rh-explore-title"><div class="rh-section-title"><div><p class="rh-eyebrow">Tournament navigation</p><h2 id="rh-explore-title">Explore the Tournament</h2><p class="rh-section-copy">Direct access to comprehensive tournament guides with fast links.</p></div></div><nav class="rh-feature-grid" aria-label="Explore CPL 2026">${tiles.map(([icon, title, text, href]) => `<a href="${href}" class="rh-feature-tile"><i data-lucide="${icon}" aria-hidden="true"></i><strong>${title}</strong><span>${text}</span><b><i data-lucide="arrow-right" aria-hidden="true"></i></b></a>`).join("")}</nav></section>`;
}
...
function renderPlayerIndex() {
  ...
  return `<section class="rh-player-index" aria-labelledby="rh-player-index-title">
    <div class="rh-player-index-copy"><p class="rh-eyebrow">Player directory</p><h2 id="rh-player-index-title">Browse Players and Squads</h2><p>Filter player profiles by playing role and T20 squad lists.</p><a href="/players/">Explore all players <i data-lucide="arrow-right" aria-hidden="true"></i></a></div>
...
function renderVenues(venues) {
  ...
  return `<article class="rh-panel rh-venues" aria-labelledby="rh-venues-title"><div class="rh-panel-heading"><div><h2 id="rh-venues-title">Top Venue Guides</h2><p class="rh-section-copy">Eight Caribbean grounds host 2026 tournament matches with stadium guides, seating, and fixtures.</p></div><a href="/venues/">View all venues <i data-lucide="arrow-right" aria-hidden="true"></i></a></div><div class="rh-venue-grid">${featuredVenues.map((venue, index) => `<a class="${index === 0 ? "featured" : ""}" href="/venues/${escapeHtml(venue.slug)}/"><img src="${escapeHtml(venue.image)}" alt="${escapeHtml(venue.name)} cricket ground" width="640" height="360" loading="lazy" decoding="async" /><span>${escapeHtml(venue.name)}</span><small>${escapeHtml(venue.location)}</small><b>View venue guide <i data-lucide="arrow-right" aria-hidden="true"></i></b></a>`).join("")}</div></article>`;
}

function renderWatch(broadcasters) {
  return `<article class="rh-panel rh-watch" aria-labelledby="rh-watch-title"><div class="rh-panel-heading"><div><h2 id="rh-watch-title">How to Watch the Tournament</h2><p class="rh-section-copy">Confirmed TV networks and streaming services broadcast match action in every region.</p></div><a href="/how-to-watch/">View Watching Guide <i data-lucide="arrow-right" aria-hidden="true"></i></a></div><div class="rh-watch-list">${broadcasters.markets.slice(0, 6).map((market) => `<div><span>${escapeHtml(market.market)}</span><b class="${market.status === "verified" ? "confirmed" : "pending"}">${market.status === "verified" ? "Confirmed" : "TBA"}</b></div>`).join("")}</div><p class="rh-watch-note"><i data-lucide="info" aria-hidden="true"></i> Broadcast details update after rights holder announcements.</p></article>`;
}
...
function renderPlanTournament(site) {
  ...
  return `<section class="rh-section rh-plan" aria-labelledby="rh-plan-title">
    <div class="rh-plan-copy"><p class="rh-eyebrow">Plan your tournament</p><h2 id="rh-plan-title">Tickets and Final Guide</h2><p class="rh-section-copy">Official ticket sellers handle match passes. Final venue details will follow official announcements.</p><div><a class="rh-button rh-button-yellow" href="/tickets/"><i data-lucide="ticket-check" aria-hidden="true"></i> Ticket guide</a><a class="rh-button rh-button-outline" href="/fixtures/"><i data-lucide="calendar-days" aria-hidden="true"></i> View fixtures</a></div></div>
    <div class="rh-plan-status">
      ...
      <article><i data-lucide="trophy" aria-hidden="true"></i><span><small>Tournament final</small><strong>${escapeHtml(finalDate)}</strong><em>Host and venue TBC</em></span></article>
    </div>
  </section>`;
}

function renderTrust(site) {
  return `<section class="rh-trust" aria-label="CPL Insider editorial information"><div><i data-lucide="file-check-2" aria-hidden="true"></i><span><strong>Sources</strong><p>Official tournament updates, team rosters, and venue news come from verified sources.</p><small>Page updated: ${escapeHtml(site.buildUpdated)}. Tournament data reviewed: ${escapeHtml(site.lastUpdated)}.</small></span></div><div><i data-lucide="pen-line" aria-hidden="true"></i><span><strong>Editor's note</strong><p>Match dates, squads, venues, and broadcasts change. Check linked sources for confirmation.</p><small>${escapeHtml(site.notAffiliated)}</small></span></div></section>`;
}
  ...
  return `<section class="rh-section rh-seo-overview" aria-labelledby="rh-seo-title">
    <div class="rh-section-title">
      <div>
        ...
        <h2 id="rh-seo-title">Caribbean Premier League 2026 Season Overview</h2>
      </div>
    </div>
    <div class="rh-seo-card">
      <div class="rh-seo-grid">
        <article class="rh-seo-block">
          ...
          <p>Caribbean Premier League 2026 runs from 7 August to 20 September 2026 across eight venues. Seven T20 franchises, including <a href="/teams/jamaica-kingsmen/">Jamaica Kingsmen</a>, <a href="/teams/trinbago-knight-riders/">Trinbago Knight Riders</a>, <a href="/teams/guyana-amazon-warriors/">Guyana Amazon Warriors</a>, <a href="/teams/barbados-tridents/">Barbados Tridents</a>, <a href="/teams/saint-lucia-kings/">Saint Lucia Kings</a>, <a href="/teams/st-kitts-nevis-patriots/">St Kitts &amp; Nevis Patriots</a>, and <a href="/teams/antigua-and-barbuda-falcons/">Antigua &amp; Barbuda Falcons</a>, battle in 39 T20 matches.</p>
        </article>
        <article class="rh-seo-block">
          <h3><i data-lucide="calendar" aria-hidden="true"></i> CPL 2026 Schedule, Live Scores & Match Centre</h3>
          <p>Match centre updates cover live scores, toss results, playing XIs, and local start times. Kensington Oval, host of CPL 2026 finals week, leads eight Caribbean venues. View the complete <a href="/fixtures/">CPL 2026 schedule</a> and <a href="/live-score/">live score match centre</a> for stadium guides.</p>
        </article>
        <article class="rh-seo-block">
          <h3><i data-lucide="users" aria-hidden="true"></i> Franchise Squads, Player Profiles & Points Table</h3>
          <p>Official <a href="/teams/">CPL 2026 team squads</a> include Nicholas Pooran, Andre Russell, Kieron Pollard, Sunil Narine, and Shai Hope with full <a href="/players/">player stats</a>. Points table tracks team wins, losses, net run rate, and standings in the live <a href="/points-table/">CPL 2026 points table</a>.</p>
        </article>
      </div>
    </div>
  </section>`;
...
function renderReferenceHome(data) {
  ...
  return `<main class="home-page rh-home" id="main-content">
    <section class="rh-hero" aria-labelledby="rh-hero-title">
      <div class="rh-hero-art"><picture><source type="image/avif" srcset="/assets/images/hero/cpl-2026-player-artwork-720.avif 720w, /assets/images/hero/cpl-2026-player-artwork-1280.avif 1280w" sizes="(max-width: 720px) 100vw, 70vw" /><source type="image/webp" srcset="/assets/images/hero/cpl-2026-player-artwork-720.webp 720w, /assets/images/hero/cpl-2026-player-artwork-1280.webp 1280w" sizes="(max-width: 720px) 100vw, 70vw" /><img src="/assets/images/hero/cpl-2026-player-artwork-1280.webp" alt="" width="1280" height="683" fetchpriority="high" decoding="async" /></picture></div>
      <div class="rh-hero-content"><p class="rh-eyebrow">Caribbean Premier League 2026</p><h1 id="rh-hero-title"><span>Caribbean Cricket's</span> <strong>Biggest Party</strong> <span>Returns</span></h1><p>Caribbean Premier League 2026 brings 39 T20 matches across eight venues. Live scores, fixtures, squads, standings, and broadcast channels update daily.</p><div class="rh-hero-actions"><a class="rh-button rh-button-yellow" href="/fixtures/">View Schedule <i data-lucide="arrow-right" aria-hidden="true"></i></a><a class="rh-button rh-button-outline" href="/teams/">Explore Teams <i data-lucide="arrow-right" aria-hidden="true"></i></a></div></div>
